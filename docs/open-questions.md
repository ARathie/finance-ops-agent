...
| What does Rackspace do with mail it scores as spam for the agent's mailbox? | **Open, and it bit us (Sep 2026).** Two messages sent straight from a tester's Gmail to the agent never arrived: no bounce, and nothing in any IMAP folder including `INBOX.spam`. The same content forwarded did arrive. That is the signature of spam filtering set to **discard** rather than quarantine. Before consultants are told to write to the agent, check the mailbox's spam settings in the Rackspace control panel, set it to quarantine or deliver rather than discard, and allowlist the consultants' domains. The backstop if one is still lost: the agent raises a `waiting_for_timesheet` item for every period it expects, and the Monday summary lists the ones with no timesheet yet. |
