...
| What is the agent's mailbox address? | **Answered (Sep 2026):** `jay@icon-technologies.com`, and the mailbox now exists at Rackspace Email. |
...
| Should the nightly backup be encrypted? | **Deferred (Sep 2026): not yet.** `fops backup` writes a plain zip holding the database, the stored timesheets, the invoice PDFs and `qbo_tokens.json` -- the QuickBooks refresh token in the clear -- and `fops serve` uploads it off the server. Anyone who can read that bucket can connect to Icon's QuickBooks company. Worth revisiting once the stored tokens are production ones. |
| Where is the public site published? | Intuit will not issue production QuickBooks credentials without a licence agreement, a privacy policy, and launch, connect and disconnect URLs, all at public https addresses (`docs/public-site/`). Write the URLs here once they are live. |
...
| Why is mail to the agent sometimes delayed by up to an hour? | **Answered (Sep 2026): greylisting.** Messages sent to the agent from a tester's Gmail took up to an hour to arrive: no bounce, nothing in any IMAP folder, and the mailbox's own spam filtering was Off -- then they turned up. That is a receiving gateway temporarily rejecting the first attempt from an unfamiliar (sending IP, sender, recipient) triple and accepting the retry. Google sends from many outbound IPs, so each new one is delayed once, which makes it look random. Nothing is lost and no setting needs changing; the agent polls every 15 minutes and reads whatever has landed. What it means in practice: **a timesheet that was just sent may not be there yet**, so a missing one is not a fault until an hour or so has passed. The backstop for a genuinely lost one is unchanged -- the agent raises a `waiting_for_timesheet` item for every period it expects, and the Monday summary lists those with no timesheet yet. |
