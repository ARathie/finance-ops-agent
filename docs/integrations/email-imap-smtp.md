...
- **Folders.** The agent creates `Agent/Processed`, `Agent/Needs Review`, and `Agent/Ignored`, under `INBOX` or at the top level depending on the server, after discovering the hierarchy delimiter with `LIST`. Moves use `UID MOVE` when the server advertises `MOVE`, else `COPY` + `\Deleted` + `EXPUNGE`. Folders are a convenience for a human looking at the mailbox; the database is the record, and a message dragged back by hand is not reprocessed.
...
- The real adapter is tested against a **local mail server in CI**, never against Rackspace: GreenMail (`greenmail/standalone` Docker image; IMAP, IMAPS, SMTP, SMTPS with test users) as a GitHub Actions service container, with Dovecot as the fallback if GreenMail proves awkward. Every case in the reading and sending sections has a test: new-message polling and last-UID advance; `UIDVALIDITY` change → full rescan with no duplicates; a redelivered message skipped by Message-ID; a message without a Message-ID; the attachment and inline-image rules; folder creation with either delimiter; `MOVE` and the copy-delete fallback; send → `250` → `accepted_at` → Sent copy; reconcile for each of the three outcomes (found, too young, `SEND_UNCERTAIN`); a rejected recipient → `SEND_FAILED`; reply matching by `In-Reply-To` and by subject; TLS verification never off.
- A live smoke test behind `FOPS_LIVE_TESTS=1` logs in to the real mailbox, lists folders, and sends one email to Kevin only. It never touches a client address.
- `tests/contract/record_mail.py` (replacing the Graph recorder) can save real server responses with secrets removed, to reproduce a Rackspace quirk in a unit test. The committed recordings are never from Icon's real mail.
