...
- [ ] **Needs a person (test):** one real email with a timesheet attached, **forwarded** from an address in `FOPS_TIMESHEET_FORWARDERS` (decision 25), is picked up by `uv run fops dry-run` (real mailbox, dry run) and produces the "timesheet received" email to Kevin.
